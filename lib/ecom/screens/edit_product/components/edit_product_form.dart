...
import 'package:swarn_holidays_business_main/ecom/components/default_button.dart';
import 'package:swarn_holidays_business_main/ecom/exceptions/local_files_handling/image_picking_exceptions.dart';
import 'package:swarn_holidays_business_main/ecom/exceptions/local_files_handling/local_file_handling_exception.dart';
...
import 'package:swarn_holidays_business_main/ecom/services/local_files_access/local_files_access_service.dart';
...
class _EditProductFormState extends State<EditProductForm> {
  ...
  Widget buildProductSearchTagsTile() {
    return ExpansionTile(
      ...
      children: [
        const Text("Your product will be searched for this Tags"),
    ...
    try {
      ...
      final productUploadFuture = newProduct
          ? ProductDatabaseHelper()
              .addUsersProduct(uid: currentUserId, product: product!)
          : ProductDatabaseHelper().updateUsersProduct(product!, rentOwnerId);
  ...
  Future<void> addImageButtonCallback({int? index}) async {
    final productDetails = Provider.of<ProductDetails>(context, listen: false);
    if (index == null && productDetails.selectedImages.length >= 5) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Max 5 images can be uploaded")));
      return;
    }
    String? path;
    String? snackbarMessage;
